...
<p id = "既然有了malloc/free，C++中为什么还需要new/delete呢？直接用malloc/free不好吗"></p>
...
<p id = "private访问和继承权限publicprotectedprivate的区别"></p>
...
try  {  可能抛出异常的语句；（检查） try 
 ...
 { 
 }  }  catch（类型名[形参名]）//捕获特定类型的异常  {  //处理1；  }  catch（类型名[形参名]）//捕获特定类型的异常  {  //处理2；  }  catch（…）//捕获所有类型的异常  {  } 
 ~~~
...
<p id = "你知道Denug和release的区别是什么吗"></p>
