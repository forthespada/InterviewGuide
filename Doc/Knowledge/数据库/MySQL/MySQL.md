...
<p  id="MyISAM和InnoDB实现B树索引方式的区别是什么"></p>
...
<p  id="说一下MySQL是如何执行一条SQL的具体步骤有哪些"></p>
...
<p  id="说一说DropDelete与Truncate的共同点和区别"></p>
...
<p  id="MySQL优化了解吗说一下从哪些方面可以做到性能优化"></p>
...
<P  id="都知道数据库索引采用B树而不是B树原因也有很多主要原因是什么"></p>

#### 11、都知道数据库索引采用B+树而不是B树，原因也有很多，主要原因是什么？

主要原因：B+树只要遍历叶子节点就可以实现整棵树的遍历，而且在数据库中基于范围的查询是非常频繁的，而B树只能中序遍历所有节点，效率太低。

<p  id="文件索引和数据库索引为什么使用B树第个问题的详细回答"></p>
...
<p  id="为什么说Btree比B树更适合实际应用中操作系统的文件索引和数据库索引"></p>
...
<p  id="一道场景题假如你所在的公司选择MySQL数据库作数据存储一天五万条以上的增量预计运维三年你有哪些优化手段"></p>
...
<p  id="MySQL常见的存储引擎InnoDBMyISAM的区别适用场景分别是"></p>
...
<p  id="SQL中的NOW和CURRENTDATE两个函数有什么区别"></p>
...
<p  id="MySQL中CHAR和VARCHAR的区别有哪些"></p>
...
<p  id="增加B树的路数可以降低树的高度那么无限增加树的路数是不是可以有最优的查找效率"></p>
...
<p  id="SQL语法中内连接自连接外连接左右全交叉连接的区别分别是什么"></p>
...
<p  id="为什么MySQL索引要使用B树而不是B树或者红黑树"></p>
...
<p  id="为什么MySQL索引适用用B树而不用hash表和B树？"></p>
...
<p  id="既然Hash比B树更快为什么MySQL用B树来存储索引呢"></p>
