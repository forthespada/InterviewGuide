...
<p  id="有MySQL不就够用了吗为什么要用Redis这种新的数据库"></p>
...
<p  id="C中的Map也是一种缓存型数据结构为什么不用Map而选择Redis做缓存"></p>
...
<p  id="定期和惰性一定能保证删除数据吗如果不能Redis会有什么应对措施"></p>
...
<p  id="假如MySQL有万数据采用Redis作为中间缓存取其中的万如何保证Redis中的数据都是热点数据"></p>
